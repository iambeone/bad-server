...
interface FilterComponentProps<TFilters> {
  ...
  onClear: () => void
  defaultValue?: TFilters
}

const Filter = <TFilters extends FiltersOrder | FiltersCustomers>({
  fields,
  onFilter,
  defaultValue,
  onClear,
}: FilterComponentProps<TFilters>) => {
  const formRef = useRef<HTMLFormElement>(null)

  const { values, handleChange } = useFormWithValidation<TFilters>(
    (defaultValue ?? {}) as TFilters,
    formRef.current
  )
  ...
  const renderController = useCallback(
    (field: Field) => {
      ...
      const rawValue = (values as unknown as Record<string, unknown>)[field.name ?? ''];
      switch (field.type) {
  ...
  useEffect(() => {
    const dv = defaultValue as FiltersOrder | undefined
    if (dv?.status) {
      const status = FILTER_ORDER_TYPES.find(
        (item) => item.value === dv.status
      )
      if (status) {
